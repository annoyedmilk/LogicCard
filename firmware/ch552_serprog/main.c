...
*                      V1.1: Added FPGA reset/enable control for LogicCard project
*                            - FPGA_RST (Port 1.0) and FPGA_EN (Port 3.3) control
*                            - Automatic FPGA reset before flash operations
*                            - Automatic FPGA enable after flash programming
*                            - LED activity indicator (active low)
*******************************************************************************/
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
// Based on SLG47910V Table 1. Block Control
// PWR (nRST) EN (nSLEEP) Description
// 0          X           Device Reset/Off State
// 1          0           Lower Power/Retention State
// 1          1           Configuration Mode

void fpga_reset()
{
    // Hold FPGA in reset (PWR=0, EN=X)
    FPGA_RST = 0;  // nRST = 0 (active low)
    FPGA_EN = 0;   // nSLEEP = 0
    mDelaymS(10);  // Hold reset for 10ms
}

void fpga_enable()
{
    // Enable FPGA for configuration (PWR=1, EN=1)
    FPGA_RST = 1;  // nRST = 1 (release reset)
    FPGA_EN = 1;   // nSLEEP = 1 (enable)
